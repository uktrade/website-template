...
$DOCKER_PATH run --rm -i --name $DOCKER_IMAGE -e IIGB_BUILD=$IIGB_BUILD -e BUCKET=$BUCKET -e DID=$DID -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e GIT_ACCESS_TOKEN=$GIT_ACCESS_TOKEN -e NO_ROBOTS=$NO_ROBOTS -e NO_CACHE=$NO_CACHE $DOCKER_IMAGE /bin/bash -c "chmod 700 /root/.ssh/id_rsa && chmod -R 777 node_modules && npm run deploy"
